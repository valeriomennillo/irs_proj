...
def calculate_feature_vector(image_path):
    url = "http://localhost:8983/solr/new_core123/custom"
    ...
    string_nums=(response.json()['feature_vector'])
    float_list = eval(string_nums)
    return float_list
...
with open(saved_model_path.replace(".h5","SOLR.json"), 'w') as filehandle:
    json.dump(sol, filehandle)